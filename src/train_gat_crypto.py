...
if USE_HOURLY_DATA:
    market_name = 'CRYPTO_1H_ALL'
    lookback_length = 168
else:
    market_name = 'CRYPTO_1D_ALL'
    lookback_length = 30
...
def save_results(best_epoch, best_valid_perf, best_test_perf, num_params):
    ...
    data_type = "hourly" if USE_HOURLY_DATA else "daily"
    result_path = os.path.join(output_dir, f'gat_crypto_{data_type}_{timestamp}.txt')
